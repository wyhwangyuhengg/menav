...
7. **导航**
   - `navigation[]`：数组；每项支持 `id`、`name`、`icon` 字符串，`id` 需唯一，并与 `pages/<id>.yml` 对应（例如 `id: common` 对应 `pages/common.yml`）
...
8. **RSS（articles Phase 2）**
   ...
   - `npm run build` 会 best-effort 刷新缓存，`npm run dev:offline` 不刷新联网缓存；无缓存时 `articles` 页面会回退到 Phase 1 的站点入口展示
...
页面配置位于 `pages/*.yml`，每个文件对应一个页面内容，并经过 `src/lib/config/schema/page.ts` 校验；schema 覆盖的页面字段包括：`title`、`subtitle`、`template`、`categories`、`sites`、`content`。文件名与导航 `id` 对应：
...
>
> 常用字段：`title`、`subtitle` 为字符串；`template` 可选 `page | projects | articles | bookmarks | content | search-results`；`categories`、`sites` 为数组；`content.file` 为字符串。
>
